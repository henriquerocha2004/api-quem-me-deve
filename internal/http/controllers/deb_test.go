...
func TestDebtController(t *testing.T) {
	t.Run("Deve criar uma divida com sucesso", func(t *testing.T) {
		service := debt.NewDebtService(memory.NewMemoryRepository())
	...
	t.Run("deve retornar os debitos de um cliente pelo ID", func(t *testing.T) {
		...
		repository.EXPECT().ClientUserDebts(gomock.Any(), clientId).Return(debts, nil)
	...
	t.Run("deve retornar um erro quando não é passado o clientId", func(t *testing.T) {
		...
		repository := mocks.NewMockRepository(ctrl)
		repository.EXPECT().ClientUserDebts(gomock.Any(), gomock.Any()).Times(0)
		...
		testCases := []struct {
			...
			{
				Url:                  "/v1/debt/",
				ErrorMessageExpected: "clientId is required",
